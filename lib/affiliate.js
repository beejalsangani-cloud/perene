...
export const RETAILERS = {
  ...
  Madewell: {
    name:           "Madewell",
    descriptor:     "denim & casual basics, $50–$250, americana/relaxed",
    network:        "cj", // Network: CJ Affiliate (Commission Junction)
    merchantId:     "",   // CJ advertiser ID for Madewell
    // MANUALLY VERIFY 2026-05-01: 403 fully blocked. The WAF redirects
    // any /search* path to "/" with a 403 — even the bare homepage 403s
    // to automation, so we can't probe the real URL shape. Keeping the
    // Salesforce-Commerce-style /search?q= as the best guess. Confirm:
    //   https://www.madewell.com/search?q=black+sandal
    buildSearchUrl: (q) => `https://www.madewell.com/search?q=${encodeURIComponent(q)}`,
  },
  "Sézane": {
    name:           "Sézane",
 ...
 * match so "sezane" still resolves to "Sézane".
 *
 * @param {string} name
